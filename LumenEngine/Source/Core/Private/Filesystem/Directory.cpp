...
LumenEngine::Filesystem::FDirectoryIterator::FDirectoryIterator ( const FPath &InDir, Bool bSkipPermissionErrors ) noexcept
    LumenEngine::Filesystem::FDirectoryIterator::FDirectoryIterator( const LumenEngine::Filesystem::FPath &InDir,
                                                                     LumenEngine::Bool bSkipPermissionErrors ) noexcept State = MakeShareable( new FInternalState() );
State = LumenEngine::MakeShared<LumenEngine::Filesystem::FDirectoryIterator::FInternalState>();
InDir.ToString(), bSkipPermissionErrors ? std::filesystem::directory_options::skip_permission_denied : std::filesystem::directory_options::none, InDir.ToString(),
    bSkipPermissionErrors ? std::filesystem::directory_options::skip_permission_denied : std::filesystem::directory_options::none,

    if ( State->ErrorCode or State->Iterator == std::filesystem::directory_iterator() ) if ( State->ErrorCode or
                                                                                             State->Iterator == std::filesystem::directory_iterator() ) bIsEnd = true;
}
else
{
    bIsEnd = false;
...
LumenEngine::Filesystem::FDirectoryIterator &
LumenEngine::Filesystem::FDirectoryIterator::operator=( const LumenEngine::Filesystem::FDirectoryIterator &InOther ) noexcept = default;
LumenEngine::Filesystem::FDirectoryIterator::Reference FDirectoryIterator::operator*() const noexcept LumenEngine::Filesystem::FDirectoryIterator::Reference
    LumenEngine::Filesystem::FDirectoryIterator::operator*() const noexcept return CurrentInfo;
}
LumenEngine::Filesystem::FDirectoryIterator::Pointer
FDirectoryIterator::operator->() const noexcept LumenEngine::Filesystem::FDirectoryIterator::Pointer
    LumenEngine::Filesystem::FDirectoryIterator::operator-> () const noexcept return &CurrentInfo;
}

LumenEngine::Filesystem::FDirectoryIterator &
FDirectoryIterator::operator++() noexcept LumenEngine::Filesystem::FDirectoryIterator &LumenEngine::Filesystem::FDirectoryIterator::operator++() noexcept
    if ( !bIsEnd && State && State -> Iterator != std::filesystem::directory_iterator() ) if (
        not bIsEnd and State and State->Iterator != std::filesystem::directory_iterator() ) State->Iterator.increment( State->ErrorCode );
if ( State->ErrorCode or State->Iterator == std::filesystem::directory_iterator() )
    if ( State->ErrorCode or State->Iterator == std::filesystem::directory_iterator() )
        bIsEnd = true;
}
    ...
    const LumenEngine::Filesystem::FPath &InPath, LumenEngine::Bool bInRecursive, const LumenEngine::Filesystem::FFilterPredicate &InFilter ) noexcept
{
    ...
    if ( bInRecursive )
    {
              ...
              std::filesystem::recursive_directory_iterator( InPath.ToString(), std::filesystem::directory_options::skip_permission_denied, ErrorCode ) )
        {
            ...
            {
                auto InfoResult = LumenEngine::Filesystem::FFile::GetInfo( LumenEngine::Filesystem::FPath( DirectoryEntry.path().string() ) );
                if ( InfoResult && ( !InFilter or InFilter( InfoResult.value() ) ) )
                {
                    FileInfos.push_back( InfoResult.value() );
                }
            }
    ...
    {
        for ( auto DirectoryIterator = LumenEngine::Filesystem::FDirectoryIterator( InPath ); DirectoryIterator != DirectoryIterator.End(); ++DirectoryIterator )
        {
            if ( not DirectoryIterator->IsDirectory() and ( not InFilter or InFilter( *DirectoryIterator ) ) )
            {
                FileInfos.push_back( *DirectoryIterator );
            }
        }
    }
    ...
    const LumenEngine::Filesystem::FPath &InPath, LumenEngine::Bool bInRecursive, const LumenEngine::Filesystem::FFilterPredicate &InFilter ) noexcept
{
    ...
    else
    {
        for ( auto DirectoryIterator = LumenEngine::Filesystem::FDirectoryIterator( InPath ); DirectoryIterator != DirectoryIterator.End(); ++DirectoryIterator )
        {
            if ( DirectoryIterator->IsDirectory() and ( not InFilter or InFilter( *DirectoryIterator ) ) )
            {
                DirectoryInfos.push_back( *DirectoryIterator );
            }
        }
    }
